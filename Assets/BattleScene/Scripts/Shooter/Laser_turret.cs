...
public class Laser_turret : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (transform.parent.transform.parent.tag == "Player")
        {
            ...
            GameObject.FindGameObjectWithTag("Player").GetComponent<Fire_manager>().addTurret(this);
         
        }
        else
        {
            laser = Resources.Load("LaserEnemy") as GameObject;
            GameObject.FindGameObjectWithTag("Enemy").GetComponent<Fire_manager>().addTurret(this);
        }
        impact = Resources.Load("fire_sound") as AudioClip;
        audio = GetComponent<AudioSource>();       
         core = transform.parent.transform.parent.GetComponent<Core>();   
    }
